"""Uttar Pradesh Board (UP Board) marksheet parser."""

import re
from app.board_parsers.base import BaseMarksheetParser, ParsedMarksheet, SubjectMark


class UPBoardParser(BaseMarksheetParser):
    board_code = "UP"
    ...
    def _extract_name(self, text: str) -> str | None:
        patterns = [
            ...
            r"(?:PARIKSHARTHI\s*KA\s*NAAM|NAAM)\s*[:\-]?\s*([A-Z][A-Z\s\.]{2,50})",
        ]
        for p in patterns:
    ...
    def _extract_year(self, text: str) -> int | None:
        match = re.search(
            r"(?:EXAM(?:INATION)?|YEAR|SESSION)\s*[:\-]?\s*(20\d{2})",
            text, re.IGNORECASE,
        )
        if match:
    ...
    def _detect_exam_type(self, text: str) -> str | None:
        ...
        if "INTERMEDIATE" in text_upper or "CLASS XII" in text_upper or "CLASS 12" in text_upper:
            return "Class 12"
        if "HIGH SCHOOL" in text_upper or "CLASS X" in text_upper or "CLASS 10" in text_upper:
            return "Class 10"
        return None
    ...
    def _extract_subjects(self, text: str) -> list[SubjectMark]:
        ...
        skip_words = {
            ...
            "VISHAY", "YOGFAL", "PRAPTAK", "POORNAK",
        }

        # UP Board pattern: Subject Theory Practical Total [/Max]
