...
class TamilNaduParser(BaseMarksheetParser):
    ...
    def parse(self, ocr_text: str) -> ParsedMarksheet:
        ...
        result.roll_number = self._extract_register_number(ocr_text)
    ...
    def _extract_name(self, text: str) -> str | None:
        ...
        for p in patterns:
            ...
            if match:
                ...
                name = re.sub(r"\s*(FATHER|MOTHER|REGISTER|REG|DATE|DOB).*", "", name, flags=re.IGNORECASE)
                return name.title()
        return None

    def _extract_register_number(self, text: str) -> str | None:
        patterns = [
            r"REG(?:ISTER|ISTRATION)?\s*(?:NO|NUMBER|NUM)\s*[:\-]?\s*([A-Z0-9]{5,20})",
            r"REGISTER\s*[:\-]?\s*([A-Z0-9]{5,20})",
            r"ROLL\s*(?:NO|NUMBER)\s*[:\-]?\s*(\d{5,15})",
            r"EMIS\s*(?:NO|NUMBER|ID)\s*[:\-]?\s*(\d{5,20})",
        ]
        for p in patterns:
            match = re.search(p, text, re.IGNORECASE)
            if match:
    ...
    def _extract_year(self, text: str) -> int | None:
        match = re.search(
            r"(?:EXAM(?:INATION)?|YEAR|MARCH|APRIL|JUNE|OCTOBER)\s*[:\-]?\s*(20\d{2})",
            text, re.IGNORECASE,
        )
        if match:
    ...
    def _detect_exam_type(self, text: str) -> str | None:
        ...
        if "HSC" in text_upper or "HIGHER SECONDARY" in text_upper or "CLASS XII" in text_upper:
            return "Class 12"
        if "SSLC" in text_upper or "CLASS X" in text_upper or "CLASS 10" in text_upper:
            return "Class 10"
        return None
    ...
    def _extract_subjects(self, text: str) -> list[SubjectMark]:
        ...
        skip_words = {
            ...
            "OBTAINED", "MAXIMUM", "MAX", "GRADE", "AGGREGATE",
        }

        # TN pattern: Subject Marks/Max
        ...
        # Alternate: Subject Code Subject Marks [/Max]
        if not subjects:
